...
wait(2)
while True:
    option, index = pick(['Encypher', "Decypher", "DECHead", "Decypher-BRUTE", "Pause (look at your console for a second)", "Exit"],
                         title="Choose a tool you're going to use.")
    if option == "Encypher":
        ...
        if option == "File":
            ...
            comment = str(
                input('Enter comment to encrypted file header (leave it empty to make it without any comments):'))
            sign = str(
                input('Enter signature to encrypted file header (leave it empty to make it without any signatures):'))
            Encrupted = None
            ...
            target.close()
            con.print(f'Successful Encryption complete. File saved as {outfile}.',
                      style="green")
            wait(5)
        ...
        try:
            ...
            con.print('DECYPHER SHUTTING DOWN')
            pass
        else:
            ...
            try:
                ...
                con.print('DECYPHER SHUTTING DOWN -- PASSWORD ERROR', style="red")
                wait(3)
            else:
                ...
                with open(outfile, 'a+') as f:
                    f.write(Decypher)
                print(Decypher)
                con.print(f'Successful Decryption complete. Decrypted file dropped as {outfile}.',
                          style="green")
                print('HEADERS')
                print('-------------------------------------------------')
        ...
        print('-------------------------------------------------')
        con.print(
            f'Emergency memory reset will now commence. Bruteforce complete. File saved in your DECTools folder as {outfile}.',
            style="red")
        wait(5)
    elif option == "Pause (look at your console for a second)":
        os.system('pause')
    elif option == "Exit":
        exit()